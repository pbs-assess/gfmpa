...
trawl <- readRDS("data-generated/syn-grid-w-restr.rds") %>%
  filter(year == 2018) %>%
  select(-year)
...
zoomed_out <- world |>
  ...
  coord_sf(
    xlim = XLIM + c(10, -10), ylim = YLIM + c(10, -10),
    crs = 4326
  ) +
  xlab("") +
  ylab("") +
...
# asis
trawl_asis <- readRDS("data-generated-ALL/syn-grid-w-restr.rds") %>%
  filter(year == 2018) %>%
  select(-year) |>
  filter(restricted)
hbll_asis <- readRDS("data-generated-ALL/hbll-n-grid-w-restr.rds") %>%
  filter(year == 2019) %>%
  select(-year) |>
  filter(restricted)
...
(g <- all %>% mutate(x = X, y = Y, X = X * 1000, Y = Y * 1000) %>%
  ...
  geom_sf(
    data = filter(Hecate, Zone_Type %in% c(
      "AMZ", # "CPZ" # AMZ = buffer, CPZ = main protected area?
