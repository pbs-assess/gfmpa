...
- include depth effect? (for now, no, just a spatial random field)
- what to do with time in conditioning model? s(year)? AR1? linear? independent? (for now s(year, k = 5))
- which tests to do after to assess recovery? (a bunch of GLMs etc. are here)
- what time spans to check? (starting with all historical HBLL N; 13 years; 7 survey years)
- what possible 'recoveries' to check? (starting with 5% per year)
- fix vs. simulate spatial and/or spatiotemporal random effects? (for now fixing spatial but not spatiotemporal fields)
- all MPAs together and/or look at some individual ones? (starting with all)
- which species to work with (starting with dogfish for no good reason other than lots of data)
