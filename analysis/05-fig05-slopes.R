...
make_ts_plot <- function(dat, k = 8, method = "loess", se = TRUE, center = FALSE) {
  ...
  g <- ggplot(dat, aes(year, log10(est), colour = type, fill = type)) +
    geom_pointrange(
      mapping = aes(ymin = log10(lwr), ymax = log10(upr)),
...
select(surv_dat, survey_abbrev, year, restricted, X, Y) |>
  distinct() |>
  group_by(survey_abbrev, year) |>
  summarise(prop = mean(restricted)) |>
  ggplot(aes(year, prop, colour = survey_abbrev)) +
  geom_point() +
