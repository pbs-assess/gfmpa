...
down_sample <- function(x, seed = 1) {
  ...
  remove <- group_by(x, grouping_code) |>
    slice_sample(n = n_per_stratum) |>
    ungroup()
  new <- anti_join(x, remove,
    by =
      join_by(species_common_name, survey_abbrev, year, X, Y, fishing_event_id)
  )
...
z2 |>
  ...
  scale_size_area() +
  geom_point(
    data = filter(survey_data, survey_abbrev == s, species_common_name == sp, restricted == TRUE),
    colour = "red", pch = 21
  )
