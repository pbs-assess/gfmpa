...
### Approaches to indices when restrictions are in place:
...
(ref:indices) Survey indices calculated from a geostatistical model for a selection of species (ordered by taxonomic group) and with a range of relative abundances and certainty regarding their population trajectories. Grey lines are based on the original (status quo) index based on all the survey samples. The dotted line represents the for the whole survey area while the solid line is the estimate within the the proposed restricted areas (MPAs). Coloured solid lines represent indices based on survey samples outside the proposed restricted areas only. The extrapolated index includes estimated relative abundance inside the restricted areas but without any samples from those areas. The shrunk index only estimates the relative abundance for unrestricted parts of the survey grid. Shaded areas indicate the 95% CIs.

```{r indices, out.width="0.8\\textwidth", fig.cap="(ref:indices)"}
include_graphics(here("figs/index-geo-restricted-highlights.pdf"))
```

```{r, out.width="0.8\\textwidth", fig.cap="Relative error by year for the same selection of species. Relative error is calculated as the difference between the restricted index estimate (either extrapotated or shrunk) and the status quo index as a proportion of the total status quo index in that year. "}
...
```{r, out.width="\\textwidth", fig.cap="The relationship between the slope of CV ratio through time and the change in the proportion of a species' abundance or biomass estimated to be inside the restricted areas for each survey. The top left quadrant contains species that ... The bottom right quadrant contains species that..."}
include_graphics(here("figs/explore-all-slopes.pdf"))
```


<!-- ```{r, out.width="0.9\\textwidth", fig.cap=""} -->
<!-- include_graphics(here("figs/explore-abs-slope.pdf")) -->
<!-- ``` -->
...
```{r, out.width="\\textwidth", fig.cap="SYN HS indices calculated from a geostatistical model."}
include_graphics(here("figs/index-hs-geo-restricted.pdf"))
```

```{r, out.width="\\textwidth", fig.cap="SYN QCS indices calculated from a geostatistical model."}
include_graphics(here("figs/index-qcs-geo-restricted.pdf"))
```

```{r, out.width="\\textwidth", fig.cap="SYN WCHG indices calculated from a geostatistical model."}
include_graphics(here("figs/index-wchg-geo-restricted.pdf"))
```

```{r, out.width="\\textwidth", fig.cap="HBLL N indices calculated from a geostatistical model."}
