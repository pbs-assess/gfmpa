...
geo_wrapper <- function(trawl_dat) {
  ...
  trawl_dat$density <- trawl_dat$density_ppkm2
  ...
  coords$X <- coords$X / 1000
  coords$Y <- coords$Y / 1000
  .grid <- expand_prediction_grid(coords, years = unique(trawl_dat$year))
  set.seed(1)
  pred <- try({
...
saveRDS(index_syn_hbll, file = "index-hbll-geo.rds")

plan(sequential)

index_syn_hbll <- readRDS("index-hbll-geo.rds")

index <- filter(index_syn_hbll, !is.na(est), !is.na(se))
